ra.core.notification_center = ra.core.notification_center or {
    enabled = true,
    width = 250,
    height = 60,
    gap = 10,
...
--- Example:
--- ```lua
--- ra.core.notification_center:add_notification([[<p style="color: red">Warning!</p>]])
--- ```
function ra.core.notification_center:add_notification(text, duration)
...
function ra.core.notification_center:print_notifications()
...
function ra.core.notification_center:print_single_notification(last_y, notification)
    local window_name = "ra_notification_" .. notification.id
    ...
    deleteLabel("ra_notification_console_" .. notification.id)
    local label = Geyser.Label:new({
        name = "ra_notification_console_" .. notification.id,
        x = 0, y = 0,
    ...
    for _, element in pairs({label, close_label}) do       
        setLabelClickCallback(element.name, "ra_notification_center_close_notification", notification)
...
function ra_notification_center_close_notification(notification)
    notification.hidden = true
    ra.core.notification_center:print_notifications()
end
